...
class ObjectRemove():
    ...
    def run(self, image_path=None,coordinates=None):
        '''
        Main run program 
        '''

        if coordinates is not None and coordinates!=[0,0,0,0,(0,0)]:
            ...
            #read in image and transform
            print('Reading in image {}'.format(image_path))
            images = self.preprocess_image(image_path)
            self.image_orig = images
    ...
    def user_click(self):
        ...
        def brush(event, x, y, flags, param):
            ...
            if event == cv2.EVENT_LBUTTONDOWN:
                ...
                if draw:
                    cv2.line(img, (ref_point[0], ref_point[1]), (x, y), (0, 255, 0), 2)
                    ref_point = [x, y]
        ...
        if len(nonzeros[0]) > 0:
            min_x = np.min(nonzeros[1])
            min_y = np.min(nonzeros[0])
            max_x = np.max(nonzeros[1])
            max_y = np.max(nonzeros[0])

            ref_point = [min_x, min_y, max_x, max_y, (max_x, max_y)]
        print(ref_point)
        return ref_point
